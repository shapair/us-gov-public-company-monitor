...
export default function ForeignHoldingsChart({ filers = [], filingTypes = [], limit = 20 }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        filers.forEach((f) => params.append('filer', f))
        filingTypes.forEach((t) => params.append('filing_type', t))
        const res = await fetch(`${API_BASE}/foreign-holdings/stats/by-ticker?${params}`)
  ...
  if (data.length === 0) return <div className="apple-card p-10 text-apple-ink-muted-48 text-apple-body">No foreign holding data available.</div>

  return (
    <div className="apple-card p-6">
      <h3 className="apple-section-title mb-1">Top Tickers by Foreign Holding Value</h3>
